...
PARAM <- list()
PARAM$experimento <- "ZZ7710_11"
PARAM$exp_input <- "HT7510_11"
...
# genero un vector de una cantidad de PARAM$semillerio  de semillas,
# buscando numeros primos al azar
# genero TODOS los numeros primos entre 100k y 1M
primos <- generate_primes(min = 100000, max = 1000000)
# seteo la semilla que controla al sample de los primos
set.seed(PARAM$semilla_primos)
# me quedo con PARAM$semillerio  primos al azar
ksemillas <- sample(primos)[1:PARAM$semillerio]
...
GrabarOutput()
# escribo parametros utilizados
write_yaml(PARAM, file = "parametros.yml")
